...
# ==========================================
# 1. Solar PV Model (Physically Accurate)
# ==========================================

class SolarPVModel:
    def __init__(self, capacity_mw, location='Tripoli'):
        self.capacity_mw = capacity_mw
        self.location = location
        
    def get_hourly_generation(self, day_of_year, hour, dust_factor=1.0):
        # Calculate daylight hours based on season (Summer vs Winter)
        daylight_hours = 12 + 2.5 * np.sin(np.radians((360/365) * (day_of_year - 81)))
        sunrise = 12 - (daylight_hours / 2)
        sunset = 12 + (daylight_hours / 2)
        
        # If it is night, generation is zero
        if hour < sunrise or hour > sunset:
            return 0.0
            
        # Sine curve for solar irradiance
        hour_angle = np.pi * (hour - sunrise) / (sunset - sunrise)
        irradiance_ratio = np.sin(hour_angle)
        
        # Final generation calculation (Linear derating based on dust)
        generation = self.capacity_mw * irradiance_ratio * dust_factor
        
        # Temperature derating (Libyan summer heat reduces efficiency by ~12%)
        temp_derating = 0.88 
...
class OptimalDispatchOptimizer:
    ...
    def optimize_24h(self, day_of_year, dust_factor=1.0):
        ...
        for hour in range(24):
            ...
            # The Digital Twin logic: Use free solar first, then fill the rest with gas
            ...
            solar_cost = solar_gen * 0.002  # LCOE for solar (very low)
...
solar_capacity = st.sidebar.slider("القدرة الشمسية (MW)", 0, 500, 200)
...
# --- FIX APPLIED HERE: Slider range changed from 0.5 to 0.1 ---
dust_factor = st.sidebar.slider(
    "عامل نظافة الألواح (1.0 = نظيف، 0.1 = عاصفة رملية)", 
    0.1, 1.0, 0.85
)

# Dynamic label to show the physical meaning of the dust factor
if dust_factor >= 0.9:
    dust_status = "✨ نظيف جداً (بعد الصيانة)"
elif dust_factor >= 0.7:
    dust_status = "☁️ غبار عادي"
elif dust_factor >= 0.4:
    dust_status = "🌫️ تراكم غبار شديد"
else:
    dust_status = "🌪️ عاصفة رملية"
    
st.sidebar.markdown(f"**الحالة الحالية:** {dust_status}")

# Default daily load profile (MW)
default_load = [
...
day_of_year = st.sidebar.slider("اليوم في السنة (1-365)", 1, 365, 172)  # 172 = Mid-summer
...
total_load = results['load_mw'].sum()

solar_fraction = total_solar / total_load if total_load > 0 else 0
...
st.info("""
💡 **ملاحظة هندسية:** 
- جرب سحب "عامل النظافة" إلى 0.1 (عاصفة رملية). ستلاحظ اختفاء اللون البرتقالي (الشمسي) فوراً، 
  وارتفاع اللون الأزرق (الغازي) والتكلفة الحمراء بشكل حاد. هذا يثبت قيمة أنظمة التنظيف الآلي للألواح!
""")
